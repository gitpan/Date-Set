...
$b = Date::Set->recur_by_rule( period => $a, BYMONTH => [ 10 ] );
is("$b",'19970101Z,19971001Z',
...
$b = $a->recur_by_rule( BYYEARDAY => [ 10 ] );
...
$b = $a->recur_by_rule( BYYEARDAY => [ -11 ] );
...
$b = $a->recur_by_rule( BYMONTHDAY => [ 10 ] );
...
$b = $a->recur_by_rule( BYMONTHDAY => [ -11 ] );
...
$b = $a->recur_by_rule( FREQ => 'YEARLY', UNTIL => '19980101Z', BYDAY => [ qw(-1WE) ] );
...
$b = Date::Set->recur_by_rule( RRULE => "BYMONTH=10" )->occurrences( period => $a );
is("$b",'19970101Z,19971001Z',
	"recur_by_rule() constructor is ok when called with a simple RRULE");

# recur_by_rule parses RRULEs properly.
$b = Date::Set->recur_by_rule( RRULE => "BYMONTH=10,11" )->occurrences( period => $a );
